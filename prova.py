...
def esci():
    ...
    while True:
        ...
        distance = round(distance, 2)
        if distance < 10:
            auto = connect.execute("SELECT * FROM automobili WHERE targa = %s", cam2())
            query = "INSERT INTO auto_uscite (targa, Data_ingresso, Data_uscita) VALUES (%s, %s, %s)"
            val = (auto["targa"], auto["Data_ingresso"], datetime.datetime.now())
            connect.execute(query, val)
            connect.execute("DELETE FROM automobili WHERE targa = %s", auto["targa"])
            connection.commit()
